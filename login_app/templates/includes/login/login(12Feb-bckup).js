...
login.call = function(args, callback) {
	...
	return fetch("https://"+args.url+"/api/method/login", {
		method: 'POST',
		headers: {
	...
	.then(function(response) {
		if (response.status == 401) {
			login.get_error_handler('{{ _("Invalid Login. Try again.") }}');
		}else if (response.status == 417) {
			login.get_error_handler('{{ _("Oops! Something went wrong") }}');
		}else if (response.status == 200) {
			response.json().then(function(data) {
				if(data.message == 'Logged In'){
					login.set_indicator('{{ _("Success") }}', 'green');
					var href= frappe.utils.sanitise_redirect(frappe.utils.get_url_arg("redirect-to")) || data.home_page;
					console.log(href);
					console.log("https://"+args.url+""+href);
					window.location.href = "https://"+args.url+ frappe.utils.sanitise_redirect(frappe.utils.get_url_arg("redirect-to")) || data.home_page;
...
login.get_error_handler = function(default_message) {
